...
class DriftSettingsRepository implements SettingsRepository {
  ...
  Future<String> exportPurchasesAsCsv() async {
    final purchases = await (_db.select(_db.purchases)..where((t) => t.deletedAt.isNull())).get();
    final products = await _db.select(_db.products).get();
    final productMap = {for (final p in products) p.id: p.name};

    final buffer = StringBuffer();
    buffer.writeln('id,local_date,product_name,quantity,unit,price_dzd,total_dzd');
    for (final p in purchases) {
      final name = productMap[p.productId] ?? 'Unknown';
      buffer.writeln('${p.id},${p.localDate},"$name",${p.quantity},${p.unitId},${p.priceDzd},${p.totalDzd}');
    }
    ...
    try {
      ...
      await _db.transaction(() async {
        ...
        for (final item in purchases) {
          ...
          final existing = await (_db.select(_db.purchases)..where((t) => t.id.equals(id))).getSingleOrNull();
          if (existing == null) {
            final purchasedAt = DateTime.tryParse(pu['purchasedAt'] as String? ?? '') ?? DateTime.now();
            await _db.into(_db.purchases).insert(
              PurchasesCompanion.insert(
                ...
                priceDzd: ((pu['totalDzd'] as num?)?.toInt() ?? 0) ~/ ((pu['quantity'] as num?)?.toDouble() ?? 1.0).clamp(1, 9999).toInt(),
                totalDzd: (pu['totalDzd'] as num).toInt(),
                purchasedAt: purchasedAt,
                localDate: pu['localDate'] as String? ?? purchasedAt.toIso8601String().substring(0, 10),
              ),
            );
          }
        }
